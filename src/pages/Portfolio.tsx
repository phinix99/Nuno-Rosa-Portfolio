  ...
  return (
    <main className="bg-[#fdfdfd] min-h-screen text-[#111] font-sans antialiased overflow-x-hidden">
      ...
      <nav className="w-full px-6 md:px-12 py-4 md:py-5 border-b border-neutral-200/80 flex justify-between items-center sticky top-0 bg-[#fdfdfd]/95 backdrop-blur-md z-50">
        <Link 
          to="/" 
          className="flex items-center gap-2 font-bold tracking-tight text-sm md:text-base uppercase hover:text-[#0B3175] transition-colors"
        >
        ...
        <div className="flex items-center gap-4 text-xs font-semibold uppercase tracking-wider">
          <Link to="/#about" className="hover:text-[#0B3175] transition-colors hidden sm:inline">
            About Me
          </Link>
          <Link to="/#vm-course" className="hover:text-[#0B3175] transition-colors hidden sm:inline">
            VM Course
          </Link>
          <Link 
            to="/#contact" 
            className="flex items-center gap-1 px-4 py-2 rounded-sm bg-[#0B3175] text-white hover:bg-black transition-colors"
          >
        ...
        <div className="flex flex-wrap items-center justify-start gap-2 pb-1">
          {galleryLinks.map((g) => (
            <Link 
              key={g.slug} 
              to={`/gallery/${g.slug}`}
              className="px-3.5 py-2 rounded-sm text-xs font-semibold tracking-wider uppercase transition-all bg-neutral-100 text-neutral-800 hover:bg-[#0B3175] hover:text-white border border-black/5 hover:border-transparent shadow-xs flex items-center gap-1.5 group"
            >
              <span>{g.name}</span>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="absolute inset-0 z-0 overflow-hidden">
                        ...
                        <div className="absolute inset-0 bg-[#0B3175]/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 mix-blend-overlay" />
                      </div>

                      {/* Center Badge if sub-group exists */}
                      {item.title ? (
                        <div className="relative z-10 p-3 text-center">
                          <span className="bg-white/95 text-black px-5 md:px-7 py-2 md:py-2.5 rounded-sm border border-black/10 font-sans text-xs md:text-sm font-semibold tracking-wide uppercase shadow-sm group-hover:bg-[#0B3175] group-hover:text-white group-hover:border-transparent transition-all duration-300 inline-block">
                            {item.title}
                          </span>
                        </div>
                      ) : (
