  ...
  return (
    <section className="w-full bg-white py-24 md:py-32 px-6 md:px-12 lg:px-20 border-b border-black/10">
      <div className="max-w-[1400px] mx-auto">
        ...
        {/* Clean, Borderless Rounded-Corner Video Showcase */}
        <motion.div 
        ...
        >
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4 h-3/4 bg-[#7F56D9]/15 blur-[120px] rounded-full pointer-events-none z-0" />
          ...
          <div className="relative w-full aspect-[16/9] rounded-2xl md:rounded-[2rem] overflow-hidden bg-black shadow-[0_30px_80px_rgba(0,0,0,0.15)] border border-neutral-200/50 z-10">
            <iframe 
              ...
              allowFullScreen
              title="VERO MODA Press Room Video"
            ...
            return (
              <div key={service.id} className="border-b border-black/10">
                <button 
                  ...
                  className={`w-full flex items-center justify-between py-8 md:py-12 text-left group transition-colors duration-300 ${isOpen ? 'text-[#7F56D9]' : 'hover:text-[#7F56D9]'}`}
                >
                  ...
                  <div className="relative w-4 h-4 shrink-0 flex items-center justify-center">
                    <span className={`absolute w-full h-[2px] transition-colors duration-300 ${isOpen ? 'bg-[#7F56D9]' : 'bg-black group-hover:bg-[#7F56D9]'}`}></span>
                    <motion.span 
                      animate={{ rotate: isOpen ? 0 : 90 }}
                      className={`absolute w-full h-[2px] transition-colors duration-300 ${isOpen ? 'bg-[#7F56D9]' : 'bg-black group-hover:bg-[#7F56D9]'}`}
                    />
                  </div>
                </button>
