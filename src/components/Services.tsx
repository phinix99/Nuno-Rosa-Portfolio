    ...
    <section className="w-full bg-white py-24 md:py-32 px-6 md:px-12 lg:px-20 border-b border-black/10">
      <div className="max-w-[1400px] mx-auto">
        ...
        >
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4 h-3/4 bg-[#0B3175]/15 blur-[120px] rounded-full pointer-events-none z-0" />
            ...
            return (
              <div key={service.id} className="border-b border-black/10">
                <button 
                  ...
                  className={`w-full flex items-center justify-between py-8 md:py-12 text-left group transition-colors duration-300 ${isOpen ? 'text-[#0B3175]' : 'hover:text-[#0B3175]'}`}
                >
                  ...
                  <div className="relative w-4 h-4 shrink-0 flex items-center justify-center">
                    <span className={`absolute w-full h-[2px] transition-colors duration-300 ${isOpen ? 'bg-[#0B3175]' : 'bg-black group-hover:bg-[#0B3175]'}`}></span>
                    <motion.span 
                      animate={{ rotate: isOpen ? 0 : 90 }}
                      className={`absolute w-full h-[2px] transition-colors duration-300 ${isOpen ? 'bg-[#0B3175]' : 'bg-black group-hover:bg-[#0B3175]'}`}
                    />
                  </div>
                </button>
