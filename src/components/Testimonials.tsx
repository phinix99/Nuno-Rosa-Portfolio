      ...
      <div className="max-w-[1500px] mx-auto relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-12 md:mb-14 border-b border-black/10 pb-5">
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2 text-xs md:text-sm font-bold tracking-[0.22em] text-transparent bg-clip-text bg-gradient-to-r from-[#0B3175] to-[#601DB3] uppercase">
          ...
          <div className="flex items-center gap-2.5">
            <button 
              onClick={handlePrev}
              className="w-10 h-10 flex items-center justify-center rounded-sm bg-[#FAF9FB] border border-black/10 text-black hover:bg-gradient-to-r hover:from-[#0B3175] hover:to-[#601DB3] hover:text-white hover:border-transparent transition-all shadow-xs"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-8 pt-4">
          <AnimatePresence mode="popLayout">
            {visibleTestimonials.map((t, idx) => (
              ...
              >
                <div className="w-full h-full p-6 md:p-7 rounded-sm md:rounded bg-[#FAF9FB] border border-black/10 hover:border-[#601DB3]/40 hover:shadow-[0_20px_40px_rgba(96,29,179,0.08)] transition-all duration-300 flex flex-col justify-between relative shadow-xs">
                  ...
                  <div className="absolute -top-7 left-6 flex items-center">
                    <div className="relative">
                      ...
                      <div className={`${t.avatar ? 'hidden' : 'flex'} w-14 h-14 rounded-full bg-gradient-to-br from-[#0B3175] to-[#601DB3] text-white font-bold items-center justify-center text-sm border-2 border-white shadow-md`}>
                  ...
                  {/* Top Right: Prominent Country Badge with Gradient Accent */}
                  <div className="flex justify-end items-center mb-5">
                    <span className="flex items-center gap-1.5 px-3 py-1 rounded-xs bg-gradient-to-r from-[#0B3175]/10 to-[#601DB3]/10 border border-[#601DB3]/25 text-[10px] md:text-[11px] font-bold tracking-wider text-[#601DB3] uppercase shadow-2xs">
                      <MapPin size={11} className="text-[#601DB3]" /> {t.country}
                    </span>
                  </div>

                  {/* Quote Body */}
                  <div className="flex flex-col gap-3 my-auto">
                    <MessageSquareQuote size={22} className="text-[#601DB3]/30 group-hover:text-[#601DB3]/60 transition-colors" />
                  ...
                  <div className="pt-4 mt-6 border-t border-black/10 flex flex-col gap-0.5">
                    <h3 className="font-sans text-sm md:text-base font-bold tracking-tight text-black group-hover:text-[#601DB3] transition-colors">
                      {t.name}
                    </h3>
          ...
          {testimonials.map((_, idx) => (
            <button
              ...
              className={`h-1.5 rounded-xs transition-all duration-300 ${
                idx === startIndex ? 'bg-gradient-to-r from-[#0B3175] to-[#601DB3] w-6' : 'bg-black/20 hover:bg-black/40 w-1.5'
              }`}
            />
          ))}
        </div>
