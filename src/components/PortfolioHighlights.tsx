        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-10 items-stretch">
          {pillars.map((pillar, idx) => (
            <motion.div
              ...
              className="p-6 md:p-8 rounded-sm md:rounded bg-[#FAF9FB] border border-black/10 hover:border-[#601DB3]/40 hover:shadow-[0_20px_40px_rgba(96,29,179,0.08)] transition-all duration-500 shadow-xs flex flex-col justify-between group"
            >
              {/* Top Header of Pillar */}
              <div className="flex flex-col gap-3 mb-6">
                <div className="flex items-center justify-between">
                  <span className="font-mono text-xs font-bold tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-[#0B3175] to-[#601DB3] uppercase">
                    PILLAR {pillar.num}
                  </span>
                ...
                <Link to={`/gallery/${pillar.primarySlug}`} className="group/title">
                  <h3 className="font-sans text-xl sm:text-2xl md:text-3xl font-bold tracking-tight uppercase text-black group-hover/title:text-[#601DB3] transition-colors">
                    {pillar.title}
                  </h3>
                </Link>
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {pillar.subDisciplines.map((sub, sIdx) => (
                    <Link
                      key={sIdx}
                      to={`/gallery/${sub.slug}`}
                      className="px-3 py-1.5 bg-white hover:bg-gradient-to-r hover:from-[#0B3175] hover:to-[#601DB3] text-neutral-800 hover:text-white border border-black/10 hover:border-transparent rounded-xs text-[11px] font-semibold tracking-wide uppercase transition-all duration-300 shadow-2xs"
                    >
                      {sub.name}
                    </Link>
                  ))}
